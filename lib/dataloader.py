    ...
    for index,(start,end) in enumerate(((0,train_line),(train_line,valid_line),(valid_line,num_of_time))):
        if index == 0:
            ...
            if channel == 41:
                scaler = Scaler_Chi(all_data[start:end,:,:,:])
        norm_data = scaler.transform(all_data[start:end,:,:,:])
        X_new,X,Y_new,Y,target_time_new,target_time = [],[],[],[],[],[]
        high_X,high_Y,high_target_time = [],[],[]
        for i in range(len(norm_data)-week_prior*days_of_week*one_day_period-pre_len+1):
            # 将数据的start整体向右移动一个week，由此，新生成的数据，都在该数据基础上，前推一个week，才能实现
            # 对应关系
            t = i+week_prior*days_of_week*one_day_period
            label = norm_data[t:t+pre_len,0,:,:]
            if index==0:
                ...
                if i<days_of_week*one_day_period:                   
                    label_new=norm_data[t:t+pre_len,0,:,:]
                    for week in range(week_prior):
                        period_list.append(i+week*days_of_week*one_day_period)
                        period_list_new.append(i+week*days_of_week*one_day_period)
                    for recent in list(range(1,recent_prior+1))[::-1]:
                        period_list.append(t-recent)
                        period_list_new.append(t-recent)
                else:
                    label_new = norm_data[t-days_of_week*one_day_period:t-days_of_week*one_day_period+pre_len,0,:,:] 
                    for week in range(week_prior):
                        period_list.append(i+week*days_of_week*one_day_period)
                        period_list_new.append(i+week*days_of_week*one_day_period-days_of_week*one_day_period)
                    for recent in list(range(1,recent_prior+1))[::-1]:
                        period_list.append(t-recent)
                        period_list_new.append(t-recent-days_of_week*one_day_period)                   
                feature = norm_data[period_list,:,:,:]
                feature_new = norm_data[period_list_new,:,:,:]
                X.append(feature)
                Y.append(label)
                Y_new.append(label_new)
                target_time.append(norm_data[t,1:33,0,0])
                X_new.append(feature_new)
            else:
                # t = i+week_prior*days_of_week*one_day_period # 672 for nyc

                label = norm_data[t:t+pre_len,0,:,:]          # when i=0, get norm_data[672,673,0,:,:] with shape(1,20,20) 
                period_list = []
                for week in range(week_prior):
                    period_list.append(i+week*days_of_week*one_day_period)
                for recent in list(range(1,recent_prior+1))[::-1]:
        ...
        # ipdb.set_trace()
        yield np.array(X_new),np.array(X),np.array(Y_new),np.array(Y),np.array(target_time),np.array(high_X),np.array(high_Y),np.array(high_target_time),scaler
